  ...
  return (
    <div className='grid grid-cols-1 md:pt-10 gap-4 md:gap-32 place-items-center mt-0 md:grid-cols-2 relative' >
      <div className='backp absolute -z-20'>
        ...
        {myProducts.filter(item=>item.category == 'guitar').map((item)=><div onClick={()=>navigate(`${item.price}`)} className='bg-black/90 w-60 h-60 rounded-md flex items-center lg:w-96 lg:h-[400px] relative justify-center my-10 lg:my-0 space-y-5' key={Math.random()*1000}>
          ...
          <div className='flex flex-col  items-center pr-2  z-10 space-y-0'>
            <img src={item.src} alt="ads" className='w-48  h-60 rounded-xl'/>            
