...
export default function PianoDetails() {
  ...
  return (
    <div  className='flex w-screen h-screen justify-center items-center bg-gradient-to-r from-slate-400 to-slate-950'>
        {myProducts.filter(item=>item.category == 'piano').filter(item=>item.price == price).map((item)=><div onClick={()=>navigate(`${item.price}`)} className='bg-black/60 w-60 h-60 rounded-md flex items-center lg:w-96 lg:h-[400px] relative justify-center my-10 lg:my-0 space-y-5' key={Math.random()*1000}>
